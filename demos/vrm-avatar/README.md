...
```bash
# From the XRBlocks repo root
npm ci
npm run serve -- --prefix demos/vrm-avatar
```

Then open `http://localhost:8080` in your browser and click **Run in Simulator**.

For WebXR (requires HTTPS):
```bash
ngrok http 8080
```
Open the ngrok URL in your headset browser.
...
| Point and select | Left mouse click |
| Switch mode | `Left Shift` |

Click anywhere on the grid to send the avatar to that position.
...
On device, `onSelectEnd` raycasts against `xb.core.depth.depthMesh` for accurate floor hits. In the simulator (no depth mesh), it falls back to intersecting the y=0 ground plane.

---

## Known gaps

- **`.vrma` format** — `@pixiv/three-vrm-animation` (VRM Animation format) is not used. Mixamo FBX retargeting is sufficient for walk/idle but `.vrma` would be needed for full expression-driven animation (e.g. from AI output).
