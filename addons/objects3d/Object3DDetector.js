        ...
        try {
            ...
            mutableCamera.getSnapshot = (opts) => {
                ...
                if (opts?.outputFormat === 'imageData') {
                    // A fresh copy, not the cached ImageData itself: another consumer
                    // polling the device camera while this patch is installed (e.g. an
                    // AprilTagTracker mid-calibration) may transfer the returned
                    // buffer to a Worker, which detaches it — corrupting this
                    // pipeline's own subsequent use of `snapImageData` if it were the
                    // same object.
                    return new ImageData(new Uint8ClampedArray(snapImageData.data), snapImageData.width, snapImageData.height);
                }
